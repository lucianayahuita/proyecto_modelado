<template>
  <div class="page-container">
    ...
    <div class="main-content">
      <div class="hero">
        <h3>Generador de números aleatorios con congruencia lineal</h3>
      </div>

      <div class="explanation">
        <p>Algoritmo que produce una secuencia de números pseudoaleatorios calculados mediante una ecuación lineal discontinua por partes</p>
      </div>
      ...
      <div class="div-n">
        <div class="datos">
          ...
          <div class="d4">
            <h3>c</h3>
            <input ref="c" type="number" autocomplete="off" placeholder="Incremento">
          </div>
          <div class="d5">
...
export default {
  ...
  methods: {
    ...
    verificarCamposVacios() {
      ...
      const c = this.$refs.c.value.trim();
    ...
    verificarNegativos() {
      ...
      const c = parseFloat(this.$refs.c.value);
      const n = parseFloat(this.$refs.n.value);
      
      if (x0 <= 0 || Ki <= 0 || P <= 0 || c <= 0 || n <= 0) {
    ...
    generarSecuencia() {
      ...
      if (this.verificarCamposVacios() && this.verificarNegativos() && this.verificarEnteros()) {
        const x0 = parseFloat(this.$refs.x0.value);
        const Ki = parseFloat(this.$refs.Ki.value);
        const P = parseFloat(this.$refs.P.value);
        const c = parseFloat(this.$refs.c.value);
        ...
        if (this.gcd(c, m) !== 1) {
          this.showAlert(`El valor de c=${c} no es relativamente primo con m=${m}`, 'error');
          return;
        }
        ...
        console.log(`Parámetros: Xo=${x0}, Ki=${Ki}, P=${P}, c=${c}`);
        console.log(`Valores calculados: a=${a}, g=${g}, m=${m}`);

        for (let i = 1; i <= l+1; i++) {
          let xi = ((a * anterior) + c) % m;
